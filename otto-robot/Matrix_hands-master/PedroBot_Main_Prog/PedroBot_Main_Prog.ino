...
//----------------------------------------------------------------
//-- Zowi basic firmware v2.1 for battery support
...
void setup() {
  ...
  pinMode(PIN_AButton, INPUT);
  pinMode(PIN_BButton, INPUT);
  pinMode(usb_connect, INPUT);
  ...
  //zowi.saveTrimsOnEEPROM(); //Uncomment this only one upload when you finaly set the trims.

  //Set a random seed
  randomSeed(analogRead(A6));

  //Interrumptions
  enableInterrupt(PIN_AButton, AButtonPushed, RISING);
  enableInterrupt(PIN_BButton, BButtonPushed, RISING);
  ...
  usb_val = digitalRead(usb_connect);
  if (usb_val == true) {
    Charging();
  }
  //Desactive start value
  start = false;
...
void loop() {
  ...
  if (Serial.available() > 0 && MODE != 4) {
    ...
    //disableInterrupt(PIN_AButton);
    //disableInterrupt(PIN_BButton);
...
void Charging() {
  ...
  for (;;) {
    for (int i = 0; i < 4; i ++) { // 5 fois {
  
        if (i==0) {
         zowi.putMouth(batt_40); 
        }
        else if (i==1) {
         zowi.putMouth(batt_60); 
        }
        else if (i==2) {
         zowi.putMouth(batt_80); 
        }
        else if (i==3) {
         zowi.putMouth(batt_full); 
        }
      delay(500);
    }
    usb_val = digitalRead(usb_connect);
...
if (batteryLevel < 20) {
    ...
    for (;;) {
      ...
      for (int i = 0; i < 30; i ++) { //  = 30 secondes
        zowi.putMouth(thunder);
        delay(500);
        zowi.clearMouth();
        delay(500);
